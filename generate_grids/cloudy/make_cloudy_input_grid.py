...
def load_cloudy_parameters(
    param_file="default.yaml", default_param_file="default.yaml", **kwarg_parameters
):
    """
...
def make_cloudy_input_grid(output_dir, grid, cloudy_params):
    ...
    for iZ in range(nZ):
        ...
        abundances = Abundances(
            Z=Z,
            alpha=cloudy_params["alpha"],
            CO=cloudy_params["CO"],
            d2m=cloudy_params["d2m"],
        )

        for ia in range(na):
            if cloudy_params["U_model"] == "ref":
                delta_log10Q = (
                    grid.log10Q["HI"][ia, iZ] - grid.log10Q["HI"][ia_ref, iZ_ref]
                )
                ...
                if cloudy_params["geometry"] == "spherical":
                    ...
                    print(
                        f"ERROR: do not understand geometry choice: {cloudy_params['geometry']}"
                    )
                ...
                print(
                    f"ERROR: do not understand U model choice: {cloudy_params['U_model']}"
                )
            ...
            shape_commands = ShapeCommands.table_sed(
                model_name, lam, lnu, output_dir="./data/"
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Write cloudy input files " "and submission script for " "a given SPS grid."
        )
    )
    ...
    parser.add_argument(
        ...
        "--machine",
        type=str,
        choices=["cosma7", "apollo"],
        default=None,
        help=(
            "Write a submission script for the ",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "The SPS grid(s) to run the cloudy grid on. "
    ...
    if args.sps_grid:
        for sps_grid in args.sps_grid:
            ...
            for i, (cloudy_params, cloudy_name) in enumerate(zip(c_params, c_name)):
                output_dir = make_directories(
                    synthesizer_data_dir, sps_grid, cloudy_name
                )

                print(
                    (
                        f"Generating cloudy grid for ({i}) "
