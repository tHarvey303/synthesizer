...
def create_new_grid(grid, synthesizer_data_dir):
    ...
    with h5py.File(f"{path_to_grids}/{grid}.hdf5", "w") as hf:
        ...
        with open(
            f"{path_to_cloudy_files}/{grid_name}/params.yaml", "r"
        ) as stream:
    ...
    with h5py.File(f"{synthesizer_data_dir}/grids/{grid_name}.hdf5", "a") as hf:
        ...
        for iZ, Z in enumerate(metallicities):
            ...
            for ia, log10age in enumerate(log10ages):
                ...
                nebular_continuum = (
                    spectra["nebular"][ia, iZ] - spectra["linecont"][ia, iZ]
                )
                ...
                for id_, wavelength_, emergent_ in zip(
                    ...
                    line["luminosity"][ia, iZ] = 10 ** (
                        emergent_ + dlog10Q[ia, iZ]
                    )  # erg s^-1
                    line["stellar_continuum"][ia, iZ] = np.interp(
                        wavelength_, lam, spectra["transmitted"][ia, iZ]
                    )  # erg s^-1 Hz^-1
