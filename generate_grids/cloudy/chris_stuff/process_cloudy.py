...
if params.fixed_U:
    ...
    Q = np.zeros(_grid.spectra['stellar'][:,age_mask,:].shape[:2])
    mass_ratio = np.zeros(_grid.spectra['stellar'][:,age_mask,:].shape[:2])
    for i in np.arange(NZ):
        for j in np.arange(Nage):
            _spec = _grid.spectra['stellar'][i,j] * 1.1964952e40  # erg s^-1 AA^-1
            Q[i,j] = measure_Q(_grid.lam, _spec)
    ...
    Q_ref = measure_Q(_grid.lam, _grid.spectra['stellar'][params.index_age, params.index_Z] * 1.1964952e40)
