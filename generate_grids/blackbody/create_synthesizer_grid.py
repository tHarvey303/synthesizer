...
def check_cloudy_runs(
    grid_name, synthesizer_data_dir, cloudy=None, replace=False, machine="apollo"
):
    """
        ...
        for iZ, log10Z in enumerate(log10Zs):
            for iU, log10U in enumerate(log10Us):
                ...
                if failed:
                    ...
                    with open(
                        f"{synthesizer_data_dir}/cloudy/{grid_name}/reprocess_names.txt",
                        "a",
                    ) as myfile:
    ...
    if N > 0:
        if machine == "apollo":
            apollo_submission_script(
                N, f"{synthesizer_data_dir}/cloudy/{grid_name}", cloudy
            )
        elif machine == "cosma7":
            cosma7_submission_script(
                N,
                f"{synthesizer_data_dir}/cloudy/{grid_name}",
                cloudy,
...
def add_lines(grid_name, synthesizer_data_dir, lines_to_include, normalisation=1.0):
    """
        ...
        for iT, log10T in enumerate(log10Ts):
            for iZ, log10Z in enumerate(log10Zs):
                for iU, log10U in enumerate(log10Us):
                    ...
                    nebular_continuum = (
                        spectra["nebular"][iT, iZ, iU] - spectra["linecont"][iT, iZ, iU]
                    )
                    ...
                    for id_, wavelength_, emergent_ in zip(
                        id[s], wavelength[s], emergent[s]
                    ):
                        line = lines[id_]

                        line.attrs["wavelength"] = wavelength_

                        line["luminosity"][iT, iZ, iU] = normalisation * 10 ** (
                            emergent_
                        )  # erg s^-1
                        line["stellar_continuum"][iT, iZ, iU] = np.interp(
                            wavelength_, lam, spectra["transmitted"][iT, iZ, iU]
                        )  # erg s^-1 Hz^-1
