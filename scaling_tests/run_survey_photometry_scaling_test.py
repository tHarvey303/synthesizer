...
for npart in nparts:
    ...
    for i, ngalaxies in enumerate(ngals):
        ...
        survey.add_photometric_instrument(
            filters=hst_filters, label="HST/WFC3_IR"
        )
        survey.add_photometric_instrument(
            filters=webb_filters, label="JWST/NIRCam"
        )
        ...
        for igal in range(ngalaxies):
            ...
            start = time.time()
            galaxy.generate_intrinsic_spectra(
                grid, update=True, integrated=True
            )
        ...
        print(
            "Completed N_galaxies=%d with N_part=%d in %.2f"
            % (len(galaxies), npart, times[i])
        )
...
# Plot the image
plt.savefig("../photometry_scaling_test.png", bbox_inches="tight", dpi=300)
