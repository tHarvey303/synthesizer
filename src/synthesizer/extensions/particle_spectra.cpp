...
static void spectra_loop_cic_serial(struct grid *grid, Particles *parts,
  ...
  double **part_props = parts->get_all_props(ndim);
...
#pragma omp parallel for schedule(static) num_threads(nthreads)
  ...
  for (int p = 0; p < npart; p++) {
    ...
    /* Get the grid indices and cell fractions for the particle. */
    get_part_ind_frac_cic_(part_indices, axis_fracs, dims, ndim, grid_props,
                           parts, p);
...
#pragma omp parallel num_threads(nthreads)
  {
    ...
    double *thread_spectra = synth_malloc<double>(
        (size_t)nlam * sizeof *thread_spectra, "thread_spectra");
...
PyObject *compute_particle_seds(PyObject *self, PyObject *args) {
  ...
  double *part_spectra = synth_malloc<double>(
      (size_t)npart * nlam * sizeof(double), "part_spectra");
  ...
  if (strcmp(method, "cic") == 0) {
    spectra_loop_cic(grid_props, particles, spectra, part_spectra, nthreads);
...
PyObject *compute_part_seds_with_vel_shift(PyObject *self, PyObject *args) {
  ...
  /* Allocate the spectra. */
  double *spectra =
      synth_malloc<double>((size_t)npart * nlam * sizeof(double), "spectra");
  if (spectra == NULL) {
    PyErr_SetString(PyExc_MemoryError,
                    "Could not allocate memory for spectra.");
    return NULL;
  ...
  double *integrated_spectra =
      synth_malloc<double>(nlam * sizeof(double), "integrated spectra");
