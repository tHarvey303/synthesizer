""" A module containing helper functions for interfacing with files.

Example usage:

    write_data_h5py("myfile.hdf5", "key1/key2", data, overwrite=True)
    write_attribute("myfile.hdf5", hdf, "attr_key", val)
    keys = get_names_h5py("myfile.hdf5", "Group")
    params = read_params(paramfile_path)
"""
import h5py
import numpy as np
