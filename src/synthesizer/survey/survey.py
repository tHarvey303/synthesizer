...
class Survey:
    ...
    def add_analysis_func(self, func, result_attribute, *args, **kwargs):
        """
        ...
        These functions should attach their results to the galaxy object,
        either as base level attributes or dictionaries containing the computed
        values. These attributes should be unique to the function to avoid
        overwriting existing attributes. This new attribute should match what
        is passed to the result_attribute argument.

        For example:

            ```python
            def my_analysis_func(galaxy, *args, **kwargs):
                galaxy.my_analysis_result = galaxy.some_attribute * 2
                return galaxy

            survey.add_analysis_func(my_analysis_func, "my_analysis_result")
            ```
        ...
        Args:
            ...
            result_attribute (str):
                The name of the attribute that the analysis function will
                attach to the galaxy object. This should be unique to avoid
        ...
        self._analysis_results_attr.append(result_attribute)
    ...
    def _run_extra_analysis(self):
        ...
        for func, args, kwargs in zip(
            self._analysis_funcs, self._analysis_args, self._analysis_kwargs
        ):
            if self.nthreads > 1:
                with Pool(self.nthreads) as pool:
                    pool.map(
    ...
    def _setup_output(self):
        ...
        # TODO: need to set up the user analysis functions here
