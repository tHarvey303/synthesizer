...
def load_Simba_slab(
    ...
    """Load a cuboid region of Simba simulation data into a single Galaxy object.
    ...
    with h5py.File(f"{directory}/{snap_name}", "r") as hf:
        ...
        g_masses = g_metals_all = g_metals = g_coods = g_hsml = g_dustmass = g_h2fraction = None
        ...
        if load_stars:
            ...
            star_mask = (
                (star_coords[:, 0] >= bounds[0, 0]) & (star_coords[:, 0] <= bounds[0, 1]) &
                (star_coords[:, 1] >= bounds[1, 0]) & (star_coords[:, 1] <= bounds[1, 1]) &
                (star_coords[:, 2] >= bounds[2, 0]) & (star_coords[:, 2] <= bounds[2, 1])
            )
        ...
        if load_gas:
            ...
            gas_coords = gas_coords_raw * scale_factor / h  # Convert to physical Mpc
            
            # Select gas particles within bounds
            gas_mask = (
                (gas_coords[:, 0] >= bounds[0, 0]) & (gas_coords[:, 0] <= bounds[0, 1]) &
                (gas_coords[:, 1] >= bounds[1, 0]) & (gas_coords[:, 1] <= bounds[1, 1]) &
                (gas_coords[:, 2] >= bounds[2, 0]) & (gas_coords[:, 2] <= bounds[2, 1])
            )
            ...
            g_metals = g_metals_all[:, 0] if len(g_metals_all) > 0 else np.array([])
        ...
        if load_dark_matter:
            ...
            dm_coords = dm_coords_raw * scale_factor / h  # Convert to physical Mpc
            
            # Select dark matter particles within bounds
            dm_mask = (
                (dm_coords[:, 0] >= bounds[0, 0]) & (dm_coords[:, 0] <= bounds[0, 1]) &
                (dm_coords[:, 1] >= bounds[1, 0]) & (dm_coords[:, 1] <= bounds[1, 1]) &
                (dm_coords[:, 2] >= bounds[2, 0]) & (dm_coords[:, 2] <= bounds[2, 1])
            )
            
            if np.sum(dm_mask) == 0:
                print("Warning: No dark matter particles found in specified region")
    ...
    if load_gas and g_masses is not None and len(g_masses) > 0:
        ...
        if galaxy.sf_gas_mass > 0:
            galaxy.sf_gas_metallicity = (
                np.sum(h2_masses * g_metals) / galaxy.sf_gas_mass.to(Msun).value
            )
        else:
            galaxy.sf_gas_metallicity = 0.0
    ...
    if load_dark_matter and dm_masses is not None and len(dm_masses) > 0:
        ...
        # Note: DM particles don't have velocities or metallicities in this context
        dm_particles = Particles(
            coordinates=dm_coods * kpc,
            velocities=np.zeros((len(dm_masses), 3)) * (km / s),  # No velocity data
            masses=dm_masses * Msun,
            redshift=redshift,
            softening_lengths=np.zeros(len(dm_masses)) * kpc,  # No softening data
