...
from unyt import unyt_array, yr, Gyr
...
class Continuity(Common):
    ...
    def __init__(self, logsfr_ratios=None, agebins=None,
        min_age=0*yr, max_age=1e11*yr):
        """Initialize the Continuity SFH model.

        Args:
        - logsfr_ratios (array-like, optional):
            Array of log_10(SFR_j / SFR_{j+1}) values. If None, defaults to 0.
        ...
        if len(logsfr_ratios) != nbins - 1:
            raise ValueError(f"logsfr_ratios must have {nbins-1} elements"
    ...
    def _logsfr_ratios_to_masses(self):
        ...
        coeffs = np.array([(1. / np.prod(sratios[:i])) * (np.prod(dt[1:i+1])
                            / np.prod(dt[:i])) for i in range(nbins)])
        m1 = 1 / coeffs.sum()
        return m1 * coeffs
    ...
    def _setup_sfr_interpolation(self):
        ...
        self.bin_centers = 10**((self.agebins[:, 0] + self.agebins[:, 1]) / 2)
        self.bin_edges = np.concatenate([10**self.agebins[:, 0],
                                        [10**self.agebins[-1, 1]]])
...
class ContinuityFlex(Common):
    ...
    def __init__(self, logsfr_ratio_young=0.0, logsfr_ratio_old=0.0,
        logsfr_ratios=None, fixed_young_bin=None, fixed_old_bin=None,
        min_age=0*yr, max_age=1e11*yr):
    ...
    def _logsfr_ratios_to_agebins(self):
        ...
        # Fixed bin times
        lower_time = (10**self.fixed_young_bin[1] -10**self.fixed_young_bin[0])
        ...
        dt1 = tflex / (1 + np.sum([np.prod(sfr_ratios[:(i+1)])
...
class Dirichlet(Common):
    ...
    Array of latent variables from Beta distributions.

    agebins (unyt_array):

    Array of age bin definitions.
    ...
    """
    def __init__(self, z_fraction=None, agebins=None,
        min_age=0*yr, max_age=1e11*yr):
        ...
        if len(z_fraction) != nbins - 1:
            raise ValueError(f"z_fraction must have {nbins-1} elements"
                              "for {nbins} age bins")
    ...
    def _zfrac_to_masses(self):
        ...
        if np.allclose(mass_fraction[idx], 0, rtol=1e-8):
            ...
            raise ValueError('The input z_fractions are '
                             'returning negative masses!')
...
class ContinuityPSB(Common):
    ...
    def __init__(
        ...
        if len(logsfr_ratios) != nflex - 1:
            raise ValueError(f"logsfr_ratios must have {nflex - 1} "
                            f"elements for nflex={nflex}")
        if len(logsfr_ratio_old) != nfixed:
            raise ValueError(f"logsfr_ratio_old must have {nfixed} "
    ...
    def init_from_prior(
        ...
        **kwargs,
        ...
        logsfr_ratio_old = t.rvs(df=student_t_df,
    ...
    def _setup_sfr_interpolation(self):
        ...
        self.sfrs = self.masses / dt
        self.bin_edges = np.concatenate([self.agebins[:, 0],
                                        [self.agebins[-1, 1]]])
...
class CombinedSFH(Common):
    ...
    def __init__(self, sfh_models, weights=None):
        ...
        for model in sfh_models:
            if not isinstance(model, Common):
                raise TypeError(f"Model {model} is not a valid "
                                 "SFH model instance")
    ...
    def get_sfr(self, age):
        return np.sum([model.get_sfr(age)* weight for model, weight
                        in zip(self.sfh_models, self.weights)], axis=0)
    ...
    def __add__(self, other):
        ...
        if isinstance(other, CombinedSFH):
            ...
            return CombinedSFH(self.sfh_models + [other],
