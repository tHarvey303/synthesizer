...
def untar_data(model, remove_archive=False):
    ...
    tar.close()
    if remove_archive:
        os.remove(f"{parent_model_dir}/{model}.tar")


def make_single_alpha_grid(model, ae="+00", bs="bin"):
    """make a grid for a single alpha enhancement"""

    _, version, imf = model.split(
        "_"
    )  # extract bpass version and imf from model designation. Used to make model name later
    ...
    model_name = (
        f"bpass-{version}-{bs}-{ae}_{imf}"  # this is the name of the ultimate HDF5 file
    )
    ...
    write_data_h5py(
        out_filename, "log10Q_original", data=log10Q_original, overwrite=True
    )
    write_attribute(
        out_filename,
        "log10Q_original",
    ...
    write_attribute(out_filename, "metallicities", "Description", "raw abundances")
    ...
    write_attribute(
        out_filename, "log10metallicities", "Description", "raw abundances in log10"
    )
    write_attribute(
        out_filename, "log10metallicities", "Units", "dimensionless [log10(Z)]"
    )
...
def make_full_grid(model, bs="bin"):
    ...
    model_name = (
        f"bpass-{version}-{bs}_{imf}"  # this is the name of the ultimate HDF5 file
    )
    ...
    write_data_h5py(
        out_filename, "alpha_enhancements", data=alpha_enhancements, overwrite=True
    )
    write_attribute(
        out_filename, "alpha_enhancements", "Description", "alpha enhancements in log10"
    )
