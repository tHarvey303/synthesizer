 ...
 * @param 
 */
PyObject *compute_metal_surface_dens(PyObject *self, PyObject *args) {

  const int nstar, ngas, kdim;
  const PyArrayObject *np_kernel, *np_star_pos, *np_gas_pos, *np_gas_sml;
  const PyArrayObject *np_gas_met, *np_gas_mass;

  if(!PyArg_ParseTuple(args, "OOOOOOiii", &np_kernel, &np_star_pos,
  ...
  double *los_metsds = malloc(nstar * sizeof(double));
  bzero(los_metsds, nstar * sizeof(double));
  ...
  for (int istar = 0; istar < nstar; istar++) {
    for (int igas = 0; igas < ngas; igas++) {
      ...
      if (abs(x) > gas_sml[igas] || abs(y) > gas_sml[igas]) continue;
      ...
      los_metsds[istar] += gas_mass[igas] * gas_met[igas] / sml_squ * kvalue;
...
static PyMethodDef LosMethods[] = {
  {"compute_metal_surface_dens", compute_metal_surface_dens, METH_VARARGS,
