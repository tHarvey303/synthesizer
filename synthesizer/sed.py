...
class Sed:
    ...
    def return_beta_spec(self, wv=[1250.0, 3000.0]):
        ...
        if self._spec_dims == 2:
            ...
            dummy = linregress(
                np.log10(self.lam[s]), np.log10(self.lnu[..., s])
            )
            slope = dummy[0]
    ...
    def get_fnu(self, cosmo, z, igm=None):
        ...
        self.lamz = self._lam * (1.0 + z)  # observed frame wavelength
        luminosity_distance = (
        ...
        # erg/s/Hz/cm2
        self.fnu = (
            self._lnu * (1.0 + z) / (4 * np.pi * luminosity_distance**2)
        )
    ...
    def get_broadband_fluxes(self, fc):  # broad band flux/nJy
        ...
        for f in fc.filters:
            ...
            int_num = integrate.trapezoid(self.fnu * f.t / self.nu, self.nu)
