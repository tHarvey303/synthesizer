...
class ParametricGalaxy(BaseGalaxy):
    ...
    def __str__(self):
        ...
        pstr += (
            f"log10(stellar mass formed/Msol): {np.log10(np.sum(self.sfzh.sfzh))}"
            + "\n"
        )
    ...
    def generate_lnu(self, grid, spectra_name):
        ...
        non0_inds = np.where(self.sfzh_ > 0)
        return np.sum(
            grid.spectra[spectra_name][non0_inds[0], non0_inds[1], :]
            * self.sfzh_[non0_inds[0], non0_inds[1]],
            axis=0,
        )
    ...
    def get_screen_spectra(
        self,
        grid,
        tauV=None,
        dust_curve=power_law({"slope": -1.0}),
        update=True,
        fesc=0.0,
    ):
        """
    ...
    def get_pacman_spectra(
        ...
        if fesc_LyA < 1.0:
            ...
            self.spectra["reprocessed"]._lnu = (1.0 - fesc) * (
                linecont + nebular_continuum + transmitted
            )

        else:
            self.spectra["reprocessed"]._lnu = (1.0 - fesc) * np.sum(
                grid.spectra["total"] * self.sfzh_, axis=(0, 1)
            )

        self.spectra["intrinsic"]._lnu = (
            self.spectra["escape"]._lnu + self.spectra["reprocessed"]._lnu
        )
            ...
            # else if the line is list or tuple denoting a doublet (or higher)
                ...
                for line_id_ in line_id:
                    ...
                    luminosity.append(
                        (1 - fesc)
                        * np.sum(
                            grid_line["luminosity"] * self.sfzh.sfzh,
                            axis=(0, 1),
                        )
                    )
    ...
    def get_attenuated_line(
        ...
        tauV_nebular=None,
        tauV_stellar=None,
        dust_curve_nebular=power_law({"slope": -1.0}),
        dust_curve_stellar=power_law({"slope": -1.0}),
        update=True,
    ):
        """
        ...
        fesc : float
            The Lyman continuum escape fraction, the fraction of ionising
            photons that entirely escape
        tauV_nebular : float
        ...
        for line_id, intrinsic_line in intrinsic_lines.items():
            ...
            # NOTE: the above is wrong and should be separated into stellar and
            #       nebular continuum components:
    ...
    def get_screen_line(
        ...
        dust_curve : obj (dust_curve)
            A dust_curve object specifying the dust curve for the nebular
            emission
