...
class Abundances:
    ...
    alpha_elements = ['O', 'Ne', 'Mg', 'Si', 'S',
                      'Ar', 'Ca', 'Ti']  # the alpha process elements
    ...
    def generate_abundances(self, Z, alpha=0.0, CO=0.0, d2m=False, scaling='Dopita+2013'):
        ...
        params = {'Z': Z, 'alpha': alpha, 'CO': CO,
                  'd2m': d2m, 'scaling': scaling}
...
class AbundancePattern(Abundances):
    ...
    def plot_abundances(self, show=False):
        ...
        ax.set_xticks(range(len(self.all_elements)),
                      self.names, rotation=90, fontsize=6.)
        ax.set_ylim([-12., 0.1])
...
def plot_abundance_patterns(abundance_patterns, labels=None, show=False, ylim=None):
    ...
    for a, label, ls, ms in zip(abundance_patterns, labels, ['-', '--', '-.', ':'], ['o', 's', 'D', 'd', '^']):
        ...
        ax.plot(i_, a_, lw=2, ls=ls, c='0.5',
                label=rf'$\rm {label}$', zorder=1)
    ...
    ax.legend()
    ax.set_xticks(range(len(a.all_elements)),
                  a.names, rotation=90, fontsize=6.)
