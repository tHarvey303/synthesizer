...
class Abundances:
    ...
    def metallicity(self, elements):
        ...
        return np.sum(
            [self.A[i] * 10 ** (elements[i]) for i in self.metals]
        ) / np.sum([self.A[i] * 10 ** (elements[i]) for i in self.all_elements])

    def generate_abundances(
        self, Z, alpha=0.0, CO=0.0, d2m=False, scaling="Dopita+2013"
    ):
        """
    ...
    def dust_to_metal(self, a, dep):
        ...
        return np.sum(
            [self.A[i] * (1.0 - dep[i]) * 10 ** a[i] for i in self.metals]
        ...
        for i in self.metals:
            ...
            if self.depsol[i] != 1.0:
                ...
                dep[i] = np.interp(
                    d2m,
                    np.array(
                        [0.0, self.dust_to_metal(self.sol, self.depsol), 1.0]
                    ),
...
class AbundancePattern(Abundances):
    ...
    def solar_relative_abundance(self, e, ref_element="H"):
        ...
        return (self.a[e] - self.a[ref_element]) - (
            self.sol[e] - self.sol[ref_element]
        )

    def plot_abundances(self, show=False):
        """
        ...
        ax.set_xticks(
            range(len(self.all_elements)), self.names, rotation=90, fontsize=6.0
        )
...
def plot_abundance_patterns(
    ...
    ax.legend()
    ax.set_xticks(
        range(len(a.all_elements)), a.names, rotation=90, fontsize=6.0
    )
