...
class kernel():
    ...
    def get_kernel(self):
        ...
        bins = np.arange(0, 1., 1./self.binsize)
        bins = np.append(bins, 1.)

        for ii in range(self.binsize):

            y, yerr = integrate.quad(self.integral_func(bins[ii]), 0, np.sqrt(1.-bins[ii]**2))
            kernel[ii] = y * 2.
...
def gadget_2(r):
    ...
    if r < 0.5:
        return (8./np.pi) * (1. - 6*(r*r) + 6*(r*r*r))
    elif r < 1.:
        return (8./np.pi) * 2 * ((1. - r)*(1. - r)*(1. - r))
    else:
        return 0.
