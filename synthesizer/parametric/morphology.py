    ...
    def update(self, p, cosmo=None, z=None):
        ...
        if cosmo and z:
            self.kpc_proper_per_mas = (
                cosmo.kpc_proper_per_arcmin(z).to("kpc/mas").value
            )

            if self.parameters["r_eff_kpc"]:
                self.parameters["r_eff_mas"] = (
                    self.parameters["r_eff_kpc"] / self.kpc_proper_per_mas
                )
            else:
                self.parameters["r_eff_kpc"] = (
                    self.parameters["r_eff_mas"] * self.kpc_proper_per_mas
                )

        if self.parameters["r_eff_kpc"]:
            self.model_kpc = Sersic2D_(
                amplitude=1,
                r_eff=self.parameters["r_eff_kpc"],
                n=self.parameters["n"],
                ellip=self.parameters["ellip"],
        ...
        if self.parameters["r_eff_mas"]:
            self.model_mas = Sersic2D_(
                amplitude=1,
                r_eff=self.parameters["r_eff_mas"],
                n=self.parameters["n"],
                ellip=self.parameters["ellip"],
                theta=self.parameters["theta"],
            )
