...
class BinnedSFZH:
    ...
    def __init__(self, log10ages, metallicities, sfzh, sfh_f=None, Zh_f=None):
        ...
        if len(set(self.metallicities[:-1] - self.metallicities[1:])) == 1:
            ...
            len(set(self.log10metallicities[:-1] - self.log10metallicities[1:]))
            == 1
        ):
...
def generate_sfzh(log10ages, metallicities, sfh, Zh, stellar_mass=1.0):
...
def generate_sfzh_from_array(
    log10ages, metallicities, sfh, Zh, stellar_mass=1.0
):
    """
...
class SFH:
    ...
    class Common:
        ...
        def __str__(self):
            ...
            pstr += (
                f'mean age: {self.calculate_mean_age().to("Myr"):.2f}' + "\n"
            )
            pstr += "-" * 10 + "\n"
            return pstr
        ...
        def sfr_(self, age):
            ...
            if age < self.max_age:
                return (1.0 / (self.max_age - age)) * np.exp(
                    -((np.log(self.max_age - age) - self.T0) ** 2)
                    / (2 * self.tau**2)
                )
            else:
                return 0.0
        ...
        def sfr_(self, age):
            ...
            if age < self.max_age:
                ...
                exponent = (
                    1
                    / np.sqrt(2 * np.pi)
                    * np.exp(-0.5 * ((age - self.mean) / self.sigma) ** 2)
                )
                phi_a = 0.5 * (
                    1
                    + math.erf(
                        ((self.min_age - self.mean) / self.sigma) / np.sqrt(2)
                    )
                )
                phi_b = 0.5 * (
                    1
                    + math.erf(
                        ((self.max_age - self.mean) / self.sigma) / np.sqrt(2)
                    )
                )
