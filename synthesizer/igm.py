...
class Madau96:
	...
	def T(self, z, lobs):
		...
		for l in lobs:
			...
			if l<=self.wvs[-1]*(1+z):
				Expteff = np.append(Expteff, np.exp(-(teff+ 0.25*(l/self.wvs[-1])**3*((1+z)**0.46-(l/self.wvs[-1])**0.46)+9.4*(l/self.wvs[-1])**1.5*((1+z)**0.18-(l/self.wvs[-1])**0.18)-0.7*(l/self.wvs[-1])**3*((l/self.wvs[-1])**(-1.32)-(1+z)**(-1.32))+0.023*((l/self.wvs[-1])**1.68-(1+z)**1.68))))
				continue
