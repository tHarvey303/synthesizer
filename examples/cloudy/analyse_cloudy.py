...
def plot_model_spectra(models, spectra = 'total', show = True):
...
def compare_with_grid(model, grid_name, ia = 0, iZ = 8, spectra = 'total', show = True):
    ...
    ax.plot(np.log10(spec_dict['lam']), np.log10(spec_dict[spectra]) + scaling, alpha = 0.5)
...
def compare_model_spectra(default_model, model, spectra = 'total', show = True):
    ...
    ax.plot(np.log10(lam), np.log10(spec_dict[spectra]/default_spec_dict[spectra]))
...
def plot_lines(model, show = True, filter = None):
...
def compare_lines(default_model, model, show = True, filter = None):
