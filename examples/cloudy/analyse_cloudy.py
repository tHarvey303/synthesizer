...
def plot_model_spectra(models, spectra="total", show=False):
    ...
    fig, ax = single((6.0, 3.5))

    for model in models:
        spec_dict = read_continuum(model, return_dict=True)
        ax.plot(
            np.log10(spec_dict["lam"]),
            np.log10(spec_dict[spectra]),
            lw=1,
            alpha=0.5,
        )
...
def compare_with_grid(
    model, grid_name, ia=0, iZ=8, spectra="total", show=False
):
    """compare a cloudy model with the corresponding grid-spectra"""

    fig, ax = single((5.0, 3.5))
    ...
    ax.plot(
        np.log10(spec_dict["lam"]),
        np.log10(spec_dict[spectra]) + scaling,
        alpha=0.5,
    )
...
def compare_linecont(model, show=False):
    ...
    ax.plot(
        np.log10(spec_dict["lam"]), np.log10(spec_dict["linecont"]), alpha=0.5
    )
...
def compare_model_spectra(default_model, model, spectra="total", show=False):
    ...
    ax.plot(
        np.log10(lam), np.log10(spec_dict[spectra] / default_spec_dict[spectra])
    )
