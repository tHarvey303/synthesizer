...
# `, and `BlackHoles`. To generate the intrinsic spectrum of the stellar
...
# providing our grid object as the first argument. This returns the
# spectra as an `Sed` object (see the [SED docs](../sed.ipynb)).
#
...
# Notice that these are unyt arrays with associated units. To plot the
# spectra manually we can do the following.
...
# However, we can also use the `stars.plot_spectra` method in the stars
# object (and, indeed, all other components) to plot all of the spectra
# associated with a galaxy at once.
...
#
# We restrict the age of star particles used for calculating the spectra.
# The age is specified by the `young` and `old` parameters; these default
# to `None`, but if set to a value of age (in units of Myrs) they will
# filter the star particles above or below this value. If both `young`
# and `old` are set, the code will raise an error.
...
# nebular emission for our camels galaxies. `get_spectra_nebular`
# produces the pure nebular emission
...
# `get_spectra_reprocessed` calculates the gas reprocessed spectra, which
# we refer to as the *reprocessed* spectra, assuming some escape fraction
# `fesc`. The combination of the  *reprocessed* and  *escaped* spectra is
...
# ## Dust attenuation
#
# We can apply a range of different dust models to our intrinsic spectra.
# `get_spectra_screen` applies a simple dust screen to all stellar
# particles, assuming a V band optical depth $\tau_V$
...
# `get_spectra_CharlotFall` applies the
# https://ui.adsabs.harvard.edu/abs/2000ApJ...539..718C/abstract two
# component dust screen model, with V band attenuation for young and old
# populations defined by the $\tau_V^{BC}$ and $\tau_V^{ISM}$
...
# ### Multiple galaxies
# If we want to create spectra for multiple galaxies we can use a list
